...
namespace ACT4
{
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            updateUI();
            label1.Text = "Attacking pairsss: " + getAttackingPairs(startState);
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            hTable = getHeuristicTableForPossibleMoves(currentState);
            ArrayList possibleMoves = getBestMoves(hTable);

            if (possibleMoves.Count == 0) return;


            Point move = (Point)chooseMove(possibleMoves);


            int currentAttackers = getAttackingPairs(currentState);


            executeMove(move);

            int newAttackers = getAttackingPairs(currentState);


            int deltaE = newAttackers - currentAttackers;


            if (deltaE > 0)
            {

                if (Math.Exp(-deltaE / temperature) < new Random().NextDouble())
                {

                    currentState.Y[move.X] = currentAttackers;
                }
            }
        ...
        private void SimulatedAnnealing(double initialTemperature, double coolingRate)
        {
            double temperature = initialTemperature;
            SixState bestState = new SixState(currentState);
            int bestAttackers = getAttackingPairs(currentState);
            ...
            while (temperature > 1)
            {
                ...
                Point move = (Point)chooseMove(possibleMoves);
                int attackersAfterMove = getAttackingPairs(currentState);

                int deltaE = attackersAfterMove - bestAttackers;

                if (deltaE < 0 || (Math.Exp(-deltaE / temperature) > new Random().NextDouble()))
                {
                    executeMove(move);
                    int newAttackers = getAttackingPairs(currentState);
                    if (newAttackers < bestAttackers)
                    {
                        bestAttackers = newAttackers;
